...
class Airodump(Dependency):
    ...
    def __init__(self, interface=None, channel=None, encryption=None,
        ...
        Configuration.initialize(linux=linux)
    ...
    def get_targets(self, old_targets=None, apply_filter=True, target_archives=None):
        ...
        if csv_filename is None or not Configuration.linux.exists(csv_filename):
    ...
    def get_targets_from_csv(csv_filename):
        ...
        data = Configuration.linux.readfile(csv_filename, mode='rb', encoding=None)
        encoding = chardet.detect(data)['encoding']
        if encoding == 'ascii':
            encoding = 'utf-8'

        # with open(csv_filename, 'r', encoding=encoding, errors='ignore') as csvopen:
        data = Configuration.linux.readfile(csv_filename, mode='r', encoding=encoding)
        if data:
            csvopen = data.split('\n')
            lines = []
            for line in csvopen:
